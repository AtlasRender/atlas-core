 ...
 * Project: atlas-core
 * File last modified: 21.02.2021, 16:41
 * All rights reserved.
 */
...
export const {connect} = jest.createMockFromModule("amqplib");
...
const createHeadersExchange = () => {
    ...
    return {
        ...
        getTargetQueues: (routingKey, options = {headers: null}) => {
...
const createChannel = async () => ({
    ...
    get: async (queueName, {noAck} = {noAck: null}) => {
...
// @ts-ignore
mocked(connect).mockReturnValue({
    createChannel,
    on: () => {
    },
    close: () => {
